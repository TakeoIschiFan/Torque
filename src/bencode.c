...
bencode_item *decode_bencode_int(bencode_context *context) {
    ...
    // TODO: negative numbers can be encoded by starting the string of digits
    // with
    // -
    // TODO: zero leading numbers aren't allowed except for 0 itself (i.e.
    // "i0e")
    ...
    if ((i < 0) || (i > (2 << 16))) {
        ...
        fprintf(stderr,
                "Warning: while decoding bencode int, found weird int of %d\n",
                i);
    }
...
bencode_item *decode_bencode_string(bencode_context *context) {
    ...
    if ((length < 0) || (length > (1e16))) {
        fprintf(stderr,
                "Warning: while decoding size of string, found size of %d\n",
                i);
    }
...
bencode_item *decode_bencode_dict(bencode_context *context) {
    ...
    while (!(*(context->cursor) == 'e')) {
        ...
        // NOTE: if we happen to be decoding a dict with a key "info" we store
        // its raw location in the bencode context. This is because for many
        // torrent applications we need to calculate a hash from the raw info
        // dict bencode data, and by doing it here we don't have to loop over
        // the raw data again later.
        if (!strcmp(dict->keys[dict->size], "info")) {
            if (context->_info_start_ptr == NULL) {
                ...
                context->_info_length =
                    context->cursor - context->_info_start_ptr;
            }
        } else {
            dict->values[dict->size] = decode_bencode_item(context);
        }
...
bool bencode_tests(void) {
    ...
    result = decode_bencode_cstring(
        "678:Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec "
        "placerat ultricies auctor. Donec vestibulum nibh id lectus elementum "
        "ultricies. Donec accumsan massa ipsum, a sollicitudin ex tempor non. "
        "Vestibulum ante ipsum primis in faucibus orci luctus et ultrices "
        "posuere cubilia curae; Donec aliquet sapien in mi dignissim sodales. "
        "Donec at diam et purus lacinia euismod id ut ligula. Etiam elementum "
        "consequat posuere. In eget turpis quis sapien elementum finibus. "
        "Vestibulum malesuada nulla at turpis tincidunt faucibus. Nulla sed "
        "sapien risus. In hac habitasse platea dictumst. Duis sit amet arcu "
        "tincidunt, molestie lorem in, luctus augue. Morbi in congue nulla. "
        "Aliquam.");
