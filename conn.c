...
bool connection_init_override_ip(connection_context* context, const struct in_addr* ip){
    context->_server_addr.sin_family = AF_INET;
    memcpy(&(context->_server_addr.sin_addr.s_addr), ip, sizeof(context->_server_addr.sin_addr.s_addr));
    context->_server_addr.sin_port = htons(context->port);
...
void connection_send(connection_context* context, unsigned char* data, unsigned int length){
    long sent = 0;
    while(sent < length){
         long bytes_sent = send(context->_socket_handle, data+sent, length-sent, 0);
         if (bytes_sent < 0){
...
long connection_receive(connection_context* context, unsigned char* data, unsigned int buffer_size){
    ...
    while (received < max_read){
        printf("now reading\n");
        long bytes_read = read(context->_socket_handle, data+received, max_read-received);
    ...
    if (received == max_read){
        fprintf(stderr, "Error: response too large for the buffer allocated to connection_receive");
    }
    return received;
}
...
bool http1_get(const char* host, const char* path, char* response_buffer, unsigned int response_buffer_size){
    ...
    // STEP 1. use DNS lookup (via netdb.h) to get a valid server IP for the host string.

    // todo gethostbyname is deprecated in favour of getaddrinfo, but this requires a rewrite of the socket stuff above.
    struct hostent* server = gethostbyname(host); // overwritable static data, doesnt need freeing
    if (server == NULL){
        int errsv = h_errno;
        fprintf(stderr, "Error: host %s not found. Error code %d: %s.\n", host, errsv, strerror(errsv));
        return false;
    }

    struct in_addr *first_addr = (struct in_addr *) server->h_addr_list[0]; // i think this always has at least one ip if the step above doesn't error

    // STEP 2. Setup TCP socket

    connection_context http_context = {
        .adress = host, // not a valid server ip, but we use connection_init_override_ip
        .port = 80
    };
